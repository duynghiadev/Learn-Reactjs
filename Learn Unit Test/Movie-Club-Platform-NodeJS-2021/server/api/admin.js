  ...
  connection.query(
    ...
    function (err, result) {
      if (result[0].total > 0) {
        ...
        connection.query(
          stmt,
          [username, hashedPassword, currentDateAndTime],
          function (err, result) {
            if (err) {
              res.status(400).json({
              ...
              axios
                .get(`http://${HOSTNAME}:${PORT}/admin/${result.insertId}`)
                .then((response) => {
...
app.put('/admin/password/:id', (req, res) => {
  ...
  if (oldPassword === newPassword) {
    ...
    axios
      .post('http://localhost:8000/admin/login', { username: username, password: oldPassword })
      .then((response) => {
