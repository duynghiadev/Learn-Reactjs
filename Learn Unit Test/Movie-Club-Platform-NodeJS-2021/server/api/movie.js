...
app.post('/movie', (req, res) => {
  ...
  if (title.length == 0) {
    ...
    connection.query(
      `SELECT COUNT(*) AS total FROM movie WHERE title = ?;`,
      [title],
      function (err, result) {
        if (result[0].total > 0) {
          res.status(409).json({
          ...
          connection.query(
            sql,
            [title, overview, runtime, trailerLink, poster, releaseDate],
            function (err, result) {
              if (err) {
                res.status(400).json({
                ...
                axios
                  .get(`http://${HOSTNAME}:${PORT}/movie/${result.insertId}`)
                  .then((response) => {
...
app.put('/movie/:id', (req, res) => {
  ...
  if (title.length == 0) {
    ...
    // Check the count of Movies with this Title
    connection.query(
      `SELECT COUNT(*) AS total FROM movie WHERE title = ? AND id != ?;`,
      [title, req.params.id],
      function (err, result) {
        if (result[0].total > 0) {
          res.status(409).json({
          ...
          connection.query(sqlGet, [req.params.id], function (err, movie) {
            if (err) {
              ...
              if (!movie.length) {
                ...
                connection.query(
                  sqlUpdate,
                  [title, overview, runtime, trailerLink, poster, releaseDate, req.params.id],
                  function (err) {
                    if (err) {
                      res.status(400).json({
