...
app.post('/crew', (req, res) => {
  ...
  if (name === null || name === '') {
    ...
    connection.query(
      stmt,
      [name, mainActivity, dateOfBirth, birthPlace, biography, picture, website],
      function (err, result) {
        if (err) {
          res.status(400).json({
          ...
          axios
            .get(`http://${HOSTNAME}:${PORT}/crew/${result.insertId}`)
            .then((response) => {
  ...
  connection.query(getOneStmt, [req.params.id], function (err, result) {
    if (err) {
      ...
      if (result.length) {
        ...
        connection.query(
          updateStmt,
          [name, mainActivity, dateOfBirth, birthPlace, biography, picture, website, req.params.id],
          function (err, result) {
            if (err) {
              res.status(400).json({
