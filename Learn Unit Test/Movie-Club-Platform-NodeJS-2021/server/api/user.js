      ...
      if (result[0].total > 0) {
        ...
        connection.query(sql, [username, hashedPassword], function (err, result) {
          if (err) {
            ...
            axios
              .get(`http://${HOSTNAME}:${PORT}/user/${result.insertId}`)
              .then((response) => {
...
app.put('/user/:id', (req, res) => {
  ...
  // Check the count of Users with this Username
  connection.query(
    `SELECT COUNT(*) AS total FROM user WHERE username = ? AND id != ?;`,
        ...
        connection.query(sqlGet, [req.params.id], function (err, user) {
          if (err) {
            ...
            if (!user.length) {
              ...
              connection.query(
                sqlUpdate,
                [username, firstName, lastName, gender, birthday, country, req.params.id],
                function (err) {
                  if (err) {
                    res.status(400).json({
