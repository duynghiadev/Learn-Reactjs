        ...
        $('#modalInfoContent1').append(`
                    <form id="updateAdminForm">
                        <div class="form-group form-custom">
                            ...
                            <input type="text" class="form-control" id="adminId" value="${
                              admin.id
                            }" disabled>
                            <label for="username">Username</label>
                            ...
                            <input type="text" class="form-control" id="createdAt" value="${formatDateTime(
                              admin.createdAt
                            )}" disabled>
                        </div>
                        <div class="modal-actions">
  ...
  $(document).on('click', '.adminDelete', function () {
    ...
    if (confirm('Are you sure that you want to delete this admin?')) {
      if (adminId === loggedInAdminId) {
        if (
          confirm(
            "Warning! You're about to delete the account that you're logged in with. Do you want to proceed?"
          )
        ) {
          $.ajax({
            url: `${URLPath}/admin/${adminId}`,
  ...
  data.forEach((admin) => {
    $('#results').append(`
            <div class="card adminDiv">
                <div data-id="${
                  admin.id
                }" class="card-body adminInfo" data-toggle="modal" data-target="#modal">
                    <h5 class="card-title">${admin.username}</h5>
                </div>
                <ul class="list-group list-group-flush">
                    <li class="list-group-item"><b>Created on: </b>${formatDate(
                      admin.createdAt
                    )}</li>
                </ul>
                <div class="card-actions">
                   ...
                   <div class="message">
                        ...
                        <p>${messages[i].content}
                            ...
                            <span class="messageUpdateDelete">
                                <i data-id="${messages[i].id}" data-userid="${
          messages[i].userId
        }" class="fas fa-edit messageUpdate" data-toggle="modal" data-target="#modal"></i>
                                <i data-id="${messages[i].id}" data-userid="${
          messages[i].userId
        }" class="fas fa-trash-alt messageDelete"></i>
                            </span>
                        </p>
                    </div>
                `)
