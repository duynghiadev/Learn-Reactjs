...
const StyledChat = styled.ul`
	...
	border: 1px solid rgba(53, 121, 246, .9);
	border-radius: 10px;
	...
	> li {
		...
		> p {
			...
			> .message-sender {
				font-weight: 700;
				background-color: rgba(83, 165, 81, .9);
				border-radius: 5px;
		...
		<div>
			<form onSubmit={(e) => {
				...
				if (text.trim().length > 0) {
					pushToFirebase(id, text).then(() => {/* console.info('Message sent for id', id) */ })
				}
			}}
			>
...
const Chat = (props: ChatProps): JSX.Element => {
	...
	useEffect(() => {
		...
		props.identities.forEach((identity: PlayerIdentity) => {
			...
			db.collection('users').doc(identity.id)
				.onSnapshot((doc) => {
					setMessageList((prevMessageList: Array<Message>) => {
						const rawData = doc.data() as {messages: {[key: string]: string}}
						if (rawData != null) {
							const newMessages: Array<Message> = Object.keys(rawData.messages)
								.map((messageTimestamp: string) => {
									return {
										sender: identity,
	...
	return (
		<StyledChat>
			...
			<li className="autoscroll-target" ref={ (elem) => {messagesEndRef.current = elem || undefined} }></li>
		</StyledChat>
	)
}
